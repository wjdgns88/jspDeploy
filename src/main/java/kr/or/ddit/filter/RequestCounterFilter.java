...
public class RequestCounterFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		ServletContext sc =request.getServletContext();		// application
		sc.setAttribute("requestCounter", requestCountMap);
		HttpServletRequest req = (HttpServletRequest)request;
		
		//전처리
		logger.debug(req.getRequestURI());//경로 출력
		//map객체에uri를 키로 하고, 값: 해당 uri 요청된 횟수
		...
		//다른 필터, 혹은 더이상의 필터가 없을경우 요청에 대한 servlet으로 전달
